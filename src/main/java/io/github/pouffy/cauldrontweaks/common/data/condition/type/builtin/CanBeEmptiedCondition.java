...
public enum CanBeEmptiedCondition implements CauldronCondition {
    INSTANCE;

    public static final MapCodec<CanBeEmptiedCondition> CODEC = MapCodec.unit(INSTANCE);

    @Override
    public CauldronConditionType<?> getType() {
        return CauldronConditions.CAN_BE_EMPTIED.get();
    }

    @Override
    public boolean isValid(CauldronBlockEntity cauldron, Player player, InteractionHand hand, ItemStack stack) {
        return FluidContainerHelper.canItemBeEmptied(stack);
    }
}
