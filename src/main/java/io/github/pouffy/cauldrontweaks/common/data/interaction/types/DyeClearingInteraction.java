...
import io.github.pouffy.cauldrontweaks.common.data.condition.type.ItemComponentsCondition;
import io.github.pouffy.cauldrontweaks.common.data.condition.type.RejectFluidComponentsCondition;
...
import io.github.pouffy.cauldrontweaks.common.data.result.item.type.RemoveComponentsItemResult;
...
public record DyeClearingInteraction(List<CauldronCondition> conditions, CauldronFluidResult fluidResult, Optional<ItemStack> result) implements ICauldronInteraction {
    public static final MapCodec<DyeClearingInteraction> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            CauldronCondition.CODEC.listOf().fieldOf("conditions").forGetter(DyeClearingInteraction::conditions),
            CauldronFluidResult.CODEC.fieldOf("result").forGetter(DyeClearingInteraction::fluidResult),
    ...
    public boolean test(CauldronBlockEntity cauldron, FluidStack fluidStack, Player player, InteractionHand hand, ItemStack stack) {
        ...
        conditions.add(new RejectFluidComponentsCondition(List.of(Holder.direct(DataComponents.DYED_COLOR))));
        conditions.add(new ItemComponentsCondition(DataComponentPredicate.EMPTY, List.of(Holder.direct(DataComponents.DYED_COLOR))));
    ...
    public void run(CauldronBlockEntity cauldron, FluidStack fluidStack, Player player, InteractionHand hand, ItemStack stack) {
        ...
        CauldronTweaks.LOGGER.info("Played dye clearing sound");
    }

    @Override
    public CauldronItemResult getItemResult(ItemStack usedItem, FluidStack usedFluid, Player player) {
        return new RemoveComponentsItemResult(List.of(Holder.direct(DataComponents.DYED_COLOR)));
    }
