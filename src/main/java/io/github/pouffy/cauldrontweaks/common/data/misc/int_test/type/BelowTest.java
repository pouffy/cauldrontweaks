...
public record BelowTest(int bound, boolean allowEqual) implements IntTest {

    public static final MapCodec<BelowTest> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            Codec.INT.fieldOf("bound").forGetter(BelowTest::bound),
            Codec.BOOL.optionalFieldOf("allow_equal", true).forGetter(BelowTest::allowEqual)
    ).apply(instance, BelowTest::new));

    @Override
    public IntTestType<?> getType() {
        return CauldronIntTests.BELOW.get();
    }
